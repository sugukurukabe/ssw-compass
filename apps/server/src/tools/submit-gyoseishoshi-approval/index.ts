...
export function registerSubmitGyoseishoshiApprovalTool(server: McpServer): void {
  server.registerTool(
    "submit_gyoseishoshi_approval",
    {
      ...
      description:
        "行政書士による書類承認を記録する。改正行政書士法§19 に基づき Pro tier かつ gyoseishoshi_verified=true のユーザーのみ利用可能 (L2)。" +
        "Records gyoseishoshi approval. Requires Pro + gyoseishoshi_verified (L2). Information only — does not constitute legal advice. Does not accept personal identifiers (residence card numbers, passport numbers, individual numbers).",
      inputSchema: SubmitGyoseishoshiApprovalInput,
      outputSchema: SubmitGyoseishoshiApprovalOutput,
      annotations: {
        readOnlyHint: false,
        idempotentHint: false,
        openWorldHint: false,
