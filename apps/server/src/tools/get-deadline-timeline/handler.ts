...
export const getDeadlineTimelineHandler = instrumentTool(
  ...
  async (rawArgs: unknown): Promise<CallToolResult> => {
    const args = GetDeadlineTimelineInput.parse(rawArgs);
    ...
    const deadlines = computeTimeline(args);

    const grounded = await vertexSearch({
      query: buildTimelineQuery(args),
    ...
    const summaryLines = payload.deadlines
      .map((d) => `・${d.label[args.language]} — ${d.relativeLabel[args.language]}`)
      .join("\n");
