...
- 認証 / Auth: 読み取りツールは匿名・無料（読み取り専用コネクタ）
...
### Pro tier ツール（3 種・OAuth scope + HITL ゲート / L2, ADR-024）

改正行政書士法§19 に基づき、**Pro かつ行政書士認証済み**ユーザーのみ利用可能。
匿名・無料ユーザーは scope ゲートで拒否されます。
...
> 注記 / Note: プライバシーポリシー完全版は行政書士レビュー後に公開予定（T8 で対応中）。
