# VISA COMPASS JAPAN (SSW) Design Specification v3 — v2 Supplement

> SSW Compass / 略称 SSW / 日本語通称 SSW コンパス
...
| v2 (旧) | v3 (新) | 用途 |
|---|---|---|
| SuguVisa Public | SSW Compass | 製品名 (display) |
| SuguVisa | SSW | 技術 namespace、tool prefix、server 識別子 |
| (日本語) スグビザ | SSW コンパス | 日本語表示名 (LP / disclaimer / docs) |
| `suguvisa-public/` | `ssw-public/` | monorepo root |
| `@suguvisa/server` | `@ssw/server` | workspace package |
| `@suguvisa/shared-types` | `@ssw/shared-types` | workspace package |
| `suguvisa-runtime@…` | `ssw-runtime@…` | GCP service account |
| `suguvisa-mcp` (OTel) | `ssw-mcp` | OpenTelemetry service.name |
| `ui://search-visa/...` | `ui://ssw-search/...` | UI Resource URI namespace |
| tool 名 `search_visa` | `ssw_search` (or `search_visa` 据え置き) | snake_case 維持。namespace 競合がなければ **prefix なしで `search_visa`** を推奨 (directory での tool 一覧視認性が高い)。 |

**Brand voice (LP / Directory copy):**

- タグライン (≤8 語, EN): *"The compass for Japanese visa procedures."*
- タグライン (≤14 字, JA): 「日本ビザ手続きの羅針盤。」
- 詳細 (≤25 語, EN): *"SSW Compass grounds your Japanese Specified Skilled Worker visa questions in 出入国在留管理庁 official sources. We point the way — gyoseishoshi handle the rest."*
...
```ts
// ui/ssw-checklist/src/main.tsx 抜粋
type ChecklistState = {
...
SSW Sprint 1–4 は **同期実装で十分**: `ssw_search` の Vertex AI Search 呼び出しは 3–10 秒、Cloud Run のリクエスト timeout (デフォルト 5 分、最長 60 分) 内に完了する。HTTP keep-alive と StreamableHTTPServerTransport の SSE response chunking で接続維持される。

### 22.2 採用条件

以下のいずれかが満たされた時点で採用検討:
- SEP-1686 が stable spec に取り込まれる
- SSW に重い tool が登場する: 例 `ssw_compose_full_checklist_review` (複数省庁横断 + Gemini grounding で 30 秒超)、`ssw_batch_law_diff` (法改正履歴の網羅 diff)
...
**脅威:** Cloud Run 上の SSW MCP server が攻撃者により compromise され、内部ネットワーク (GCP metadata service `169.254.169.254`、社内 GCP リソース等) へ HTTP request を送信する SSRF。匿名公開エンドポイントは特に脆弱。

**Terraform module 追加:**

```hcl
# infra/terraform/modules/egress/main.tf
resource "google_vpc_access_connector" "ssw" {
  name          = "ssw-egress"
  region        = "asia-northeast1"
  ip_cidr_range = "10.8.0.0/28"
  network       = google_compute_network.ssw.name
  min_throughput = 200
  max_throughput = 300
}

resource "google_compute_router" "ssw_router" {
  name    = "ssw-router"
  region  = "asia-northeast1"
  network = google_compute_network.ssw.name
}

resource "google_compute_router_nat" "ssw_nat" {
  name                = "ssw-nat"
  router              = google_compute_router.ssw_router.name
  region              = google_compute_router.ssw_router.region
  nat_ip_allocate_option = "MANUAL_ONLY"
  nat_ips             = [google_compute_address.ssw_nat_ip.self_link]
  source_subnetwork_ip_ranges_to_nat = "ALL_SUBNETWORKS_ALL_IP_RANGES"
}

# Cloud Run service の vpc_access に bind
# vpc_access {
#   connector = google_vpc_access_connector.ssw.id
#   egress    = "ALL_TRAFFIC"   # private + internet すべて NAT 経由
# }

# 169.254/16 (GCP metadata) と RFC1918 を Cloud Armor で deny
resource "google_compute_security_policy" "ssw_egress" {
  name = "ssw-egress-deny"
  rule {
    action   = "deny(403)"
    priority = 1000
    match {
...
SSW は Vertex AI Search / Secret Manager / Cloud Logging 以外の外部接続を必要としないため、allowlist は極めて狭い。
...
You are working on **SSW Compass (SSW)** — a public, read-only,
...
- Service account: ssw-runtime@<project>.iam (BYOSA). Never default P4SA.
...
- Cloud Run: --max-instances=20, --vpc-egress=all-traffic via ssw-egress connector.
- All resources tagged: env=prod|staging, owner=ssw, cost-center=visa-compass.
```
...
SSW 開発サーバ自体を Cursor の MCP として登録 (`.cursor/mcp.json` — **本書 24.1 で「編集禁止」と書いたが、初回セットアップでは管理者が 1 回設定**):
...
これで Cursor Agent に「SSW の `search_visa` を `特定技能2号 農業` で呼び、structuredContent.results が空でないことを確認するテストを書け」と指示できる。Agent は MCP Inspector を経由せず直接 tool を叩いて応答を見るため、iteration 速度が劇的に上がる。
...
| R20 | ネーミング誤認 (ビザコン → 婚活コン連想) | SSW Compass / SSW / SSW コンパスで brand 統一 (本書 A 章) |
...
- ✅ `ssw-public/` モノレポ scaffold (pnpm + Turborepo + Biome v2.4)
...
SSW Compass / SSW という brand は単なる呼称変更を超えて、**行政書士法 §19-1 防衛そのものを brand metaphor 化** している。「私たちは羅針盤であり、目的地まで連れて行く船ではない」というメッセージは、disclaimer の文言、tool description、directory copy、LP コピーの全レイヤで一貫させやすく、user の認知負荷を下げ、規制当局へのコミュニケーションも簡明化する。
