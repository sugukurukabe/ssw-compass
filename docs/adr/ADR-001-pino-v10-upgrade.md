...
```
pino@9.14.0
└── @ssw/server@1.0.0              (spec-declared, ^9.5.0)

pino@10.3.1
└─┬ @google-cloud/pino-logging-gcp-config@1.3.3
  └── @ssw/server@1.0.0              (transitive, ^10.3.1 wins the hoist)
...
4. Remove the `cross-env` devDependency. SSW targets macOS (dev) and Cloud
...
- SSW follows the pino v10 LTS track instead of approaching EOL on v9.
